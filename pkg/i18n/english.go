...
func addEnglish(i18nObject *i18n.Bundle) error {
	...
	return i18nObject.AddMessages(language.English,
		&i18n.Message{
			...
			Other: "Can only reword topmost commit from within lazynpm. Use shift+R instead",
		}, &i18n.Message{
			ID:    "renameCommit",
			Other: "reword commit",
		}, &i18n.Message{
			...
			Other: `Thanks for using lazynpm! Three things to share with you:

1) lazynpm now has basic mouse support!

2) If you want to learn about lazynpm's features, watch this vid:
...
3) Github are now matching any donations dollar-for-dollar for the next 12 months, so if you've been tossing up over whether to click the donate link in the bottom right corner, now is the time!`,
			...
			Other: `Lazynpm can't use "git fetch" in a private repo; use 'f' in the files panel to run "git fetch" manually`,
		}, &i18n.Message{
			ID:    "StageLines",
