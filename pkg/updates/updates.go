...
const (
	PROJECT_URL = "https://github.com/jesseduffield/lazynpm"
)
...
// example: https://github.com/jesseduffield/lazynpm/releases/download/v0.1.73/lazynpm_0.1.73_Darwin_x86_64.tar.gz
func (u *Updater) getBinaryUrl(newVersion string) (string, error) {
	...
	url := fmt.Sprintf(
		"%s/releases/download/%s/lazynpm_%s_%s_%s.%s",
		PROJECT_URL,
		newVersion,
		newVersion[1:],
...
func (u *Updater) downloadAndInstall(rawUrl string) error {
	...
	tempPath := filepath.Join(configDir, "temp_lazynpm")
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("error while trying to download latest lazynpm: %s", resp.Status)
	}
