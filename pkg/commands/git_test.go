...
import (
	...
	"github.com/jesseduffield/lazynpm/pkg/test"
...
func TestSetupRepositoryAndWorktree(t *testing.T) {
	...
	scenarios := []scenario{
		...
		},
		{
			"Setup done properly",
			func(string) (*gogit.Repository, error) {
				assert.NoError(t, os.RemoveAll("/tmp/lazynpm-test"))
				r, err := gogit.PlainInit("/tmp/lazynpm-test", false)
				assert.NoError(t, err)
				return r, nil
			},
			func(string) string { return "" },
...
func TestNewGitCommand(t *testing.T) {
	...
	scenarios := []scenario{
		...
		},
		{
			...
			func() {
				assert.NoError(t, os.RemoveAll("/tmp/lazynpm-test"))
				_, err := gogit.PlainInit("/tmp/lazynpm-test", false)
				assert.NoError(t, err)
				assert.NoError(t, os.Chdir("/tmp/lazynpm-test"))
			},
			func(gitCmd *GitCommand, err error) {
				assert.NoError(t, err)
			},
