...
func (c *GitCommand) RunSkipEditorCommand(command string) error {
	...
	lazyGitPath := c.OSCommand.GetLazynpmPath()
	cmd.Env = append(
		cmd.Env,
...
// we tell git to run lazynpm to edit the todo list, and we pass the client
// lazynpm a todo string to write to the todo file
func (c *GitCommand) PrepareInteractiveRebaseCommand(baseSha string, todo string, overrideEditor bool) (*exec.Cmd, error) {
	ex := c.OSCommand.GetLazynpmPath()
